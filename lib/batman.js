(function() {
  ...
  Batman.Controller = (function() {
    ...
    Controller.prototype.dispatch = function(action, params) {
      var oldRedirect, redirectTo, yield, _i, _len, _ref, _ref2, _ref3, _ref4;
    ...
    Controller.prototype.render = function(options) {
      ...
      if (view) {
        ...
        view.on('ready', function() {
          var _ref3;
          Batman.DOM.yieldWithName(into).replace(view.get('node'));
  ...
  Batman.DOM = {
    readers: {
      ...
      yield: function(node, key) {
        $onParseExit(node, function() {
          return Batman.DOM.yieldWithName(key).set('destinationNode', node);
        });
        return true;
      },
      contentfor: function(node, key) {
        $onParseExit(node, function() {
          return Batman.DOM.yieldWithName(key).append(node);
        });
        return true;
      },
      replace: function(node, key) {
        $onParseExit(node, function() {
          return Batman.DOM.yieldWithName(key).replace(node);
        });
        return true;
      }
    },
    ...
    yields: {},
    yieldWithName: function(name) {
      var _base;
      (_base = Batman.DOM.yields)[name] || (_base[name] = new Batman.DOM.Yield({
        name: name
      }));
      return Batman.DOM.yields[name];
    },
